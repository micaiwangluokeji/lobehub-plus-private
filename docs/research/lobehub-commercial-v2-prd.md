...
升级弹窗在不同场景复用同一个组件，但文案根据触发的功能不同而变化：

```tsx
<UpgradeModal
  feature="create_agent"  // 触发来源
  // 根据 feature 自动匹配对应文案:
  // "创建 Agent" → "升级 Pro 即可创建自定义 Agent"
  // "发布到 Discover" → "升级 Pro 即可发布到 Discover 市场"
  // "自定义服务商" → "升级 Pro 即可添加自定义 AI 服务商"
/>
```

...
### 1.2.1 商业化功能模块（v1.6 新增）

| # | 页面 | 状态 | 说明 |
|---|------|------|------|
| 16 | 支付配置 | ✅ 已完成 | 微信支付/支付宝参数配置 |
| 17 | 套餐/积分管理 | ✅ 已完成 | 套餐 CRUD + 积分配置（充值规则、赠送比例等） |
| 18 | 支付订单 | ✅ 已完成 | 支付订单列表、状态查看 |
| 19 | 退款审核 | ✅ 已完成 | 退款申请审核（批准/拒绝 + 备注） |
| 20 | 订阅管理 | ✅ 已完成 | 订阅列表/筛选/创建/编辑/取消/续订 |
| 21 | 积分交易 | ✅ 已完成 | 积分交易记录 + 手动调整积分 |
| 22 | 消费记录 | ✅ 已完成 | Token 消耗明细 + 模型/用户统计 |
| 23 | 收入仪表盘 | ✅ 已完成 | 收入概览/订阅统计/趋势分析 |
| 24 | 会员等级 | ✅ 已完成 | 会员等级定义(名称/门槛/权益) CRUD |
| 25 | 字典配置 | ✅ 已完成 | 通用 key-value 动态配置管理 |

### 1.2.2 后端新增模块

| 模块 | 文件 | 说明 |
|------|------|------|
| 支付订单 Schema | `packages/database/src/schemas/paymentOrders.ts` | payment_orders 表 |
| 会员等级 Schema | `packages/database/src/schemas/membershipLevels.ts` | membership_levels 表 |
| 退款申请 Schema | `packages/database/src/schemas/refundRequests.ts` | refund_requests 表 |
| 字典配置 Schema | `packages/database/src/schemas/dictConfigs.ts` | dict_configs 表 |
| 用户表修改 | `packages/database/src/schemas/user.ts` | 新增 membership_level_id 字段 |
| 支付订单 Model | `packages/database/src/models/paymentOrders.ts` | 订单 CRUD |
| 会员等级 Model | `packages/database/src/models/membershipLevels.ts` | 等级 CRUD |
| 退款申请 Model | `packages/database/src/models/refundRequests.ts` | 退款 CRUD |
| 字典配置 Model | `packages/database/src/models/dictConfigs.ts` | 配置 CRUD |
| topUp Router | `packages/business-server/src/lambda-routers/topUp.ts` | createOrder/queryOrder/listMyOrders |
| membershipLevel Router | `packages/business-server/src/lambda-routers/membershipLevel.ts` | 等级 CRUD |
| refundRequest Router | `packages/business-server/src/lambda-routers/refundRequest.ts` | 退款申请/审核 |
| dictConfig Router | `packages/business-server/src/lambda-routers/dictConfig.ts` | 配置 CRUD |
| 微信支付回调 | `src/app/(backend)/webapi/wxpay/notify/route.ts` | 支付通知处理 |

### 1.2.3 前端新增模块

| 组件 | 文件 | 说明 |
|------|------|------|
| 订单管理 | `src/features/Admin/Revenue/OrderList.tsx` | 支付订单列表 |
| 退款审核 | `src/features/Admin/Revenue/RefundRequestList.tsx` | 退款申请审核 |
| 会员等级管理 | `src/features/Admin/Membership/MembershipLevelList.tsx` | 等级 CRUD |
| 字典配置管理 | `src/features/Admin/Settings/DictConfigList.tsx` | 配置 CRUD |
| 角色详情 | `src/features/Admin/Roles/RoleDetail.tsx` | 角色信息+权限+成员 |

...
export const plans = pgTable('plans', {
  ...
  personalBudget: numeric('personal_budget', { mode: 'number', precision: 15, scale: 2 }).default(
    0,
  ),
  workspaceBudget: numeric('workspace_budget', { mode: 'number', precision: 15, scale: 2 }).default(
    0,
  ),
...
export const creditConfigs = pgTable('credit_configs', {
  id: text('id')
    ...
    .notNull()
    .primaryKey(),
  pricePerCredit: numeric('price_per_credit', { mode: 'number', precision: 10, scale: 4 }).default(
    0.01,
  ),
  minTopUpAmount: numeric('min_top_up_amount', { mode: 'number', precision: 10, scale: 2 }).default(
    1,
  ),
  maxTopUpAmount: numeric('max_top_up_amount', { mode: 'number', precision: 10, scale: 2 }).default(
    9999,
  ),

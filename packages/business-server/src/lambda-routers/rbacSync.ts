  ...
  syncPermissions: adminProcedure.mutation(async ({ ctx }) => {
    ...
    if (newCodes.length > 0) {
      ...
      const categoryMap: Record<string, string> = {
        agent: 'agent', ai_model: 'ai', ai_provider: 'ai',
        api_key: 'api', document: 'content', file: 'content',
        knowledge_base: 'content', message: 'chat', session: 'chat',
        session_group: 'chat', topic: 'chat', translation: 'content',
        user: 'user', rbac: 'system', workspace: 'workspace',
        workspace_member: 'workspace', workspace_audit: 'workspace',
        workspace_role: 'workspace', group: 'agent',
        billing: 'billing', credit: 'billing', subscription: 'billing', referral: 'billing',
      };
      for (const code of newCodes) {
        const prefix = code.split(':')[0];
        const cat = categoryMap[prefix] || 'other';
        await db!.insert(permissions)
          .values({ id: crypto.randomUUID?.() || `perm-${code}`, code, name: code, category: cat, isActive: true })
          .onConflictDoNothing()
          .catch(() => {});
      }

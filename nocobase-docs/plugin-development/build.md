...
把 `.tar.gz` 文件上传并解压到目标应用的 `./storage/plugins` 目录即可。详细步骤见 [安装与升级插件](https://docs.nocobase.com/cn/get-started/install-upgrade-plugins)。

### 默认启用插件

上传后插件默认不会自动激活——它会出现在「插件管理器」里，需要手动开启。如果你在维护自己的 NocoBase 应用，希望插件随应用一起默认启用，可以用 `APPEND_PRESET_BUILT_IN_PLUGINS`（追加默认内置插件）环境变量控制，用法见[让插件默认预置或默认启用](https://docs.nocobase.com/cn/plugin-development/write-your-first-plugin#%E8%AE%A9%E6%8F%92%E4%BB%B6%E9%BB%98%E8%AE%A4%E9%A2%84%E7%BD%AE%E6%88%96%E9%BB%98%E8%AE%A4%E5%90%AF%E7%94%A8%E5%8F%AF%E9%80%89)。

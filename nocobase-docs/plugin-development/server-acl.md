...
ACL（Access Control List）用于控制资源操作权限。你可以将权限赋予角色，也可以跳过角色约束直接约束权限。ACL 系统提供了灵活的权限管理机制，支持权限片段、中间件、条件判断等多种方式。

提示

ACL 对象归属于数据源（`dataSource.acl`），主数据源的 ACL 可以通过 `app.acl` 快捷访问。其他数据源的 ACL 使用方式详见 [DataSourceManager 数据源管理](https://docs.nocobase.com/cn/plugin-development/server/data-source-manager)。

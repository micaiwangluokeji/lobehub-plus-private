...
当插件需要安装下列依赖时，请确保 **版本号** 与全局依赖中 @nocobase/server 和 @nocobase/client 保持一致，否则可能导致运行时冲突。
...
对于常见的 UI 库（如 antd）、工具库（如 lodash）、数据库驱动（如 pg、mysql2），应依赖全局提供的版本，避免重复打包。

### 调试与生产环境一致

使用 devDependencies 即可保证开发与最终产物一致，避免因 dependencies 与 peerDependencies 配置不当导致的环境差异。

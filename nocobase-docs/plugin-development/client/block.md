...
- renderComponent() — 渲染区块 UI，通过 this.props 读取属性
- define() — 设置区块在「添加区块」菜单里的显示名
- registerFlow() — 添加可视化配置面板，用户可以在界面上编辑 HTML 内容
...
通常来说，如果内置的 TableBlockModel 已经满足需求，用户直接在界面上添加就行，开发者不需要做任何事。只有当你需要在 TableBlockModel 基础上做定制时，才需要继承它——比如：
...
完整的 TableBlockModel 定制示例见 做一个前后端联动的数据管理插件。

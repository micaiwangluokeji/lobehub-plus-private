...
- [编写第一个插件](https://docs.nocobase.com/cn/plugin-development/write-your-first-plugin) — 插件创建和目录结构

- [Plugin 插件](https://docs.nocobase.com/cn/plugin-development/client/plugin) — 插件入口和 `load()` 生命周期

- [Router 路由](https://docs.nocobase.com/cn/plugin-development/client/router) — `pluginSettingsManager` 设置页注册

- [Component 组件开发](https://docs.nocobase.com/cn/plugin-development/client/component/) — React 组件写法和 useFlowContext

- [i18n 国际化](https://docs.nocobase.com/cn/plugin-development/client/component/i18n) — 翻译文件写法和 `useT()` 用法
...
这会在 `packages/plugins/@my-project/plugin-settings-page` 下生成基础文件结构，包括 `src/client-v2/`、`src/server/`、`src/locale/` 等目录。详细说明见 [编写第一个插件](https://docs.nocobase.com/cn/plugin-development/write-your-first-plugin)。
...
注册后，「插件配置」菜单里会出现一个「外部服务配置」入口，顶部会有两个 tab——「API 配置」和「关于」。当菜单下只有一个页面时 tab 栏会自动隐藏，这里注册了两个页面所以会自动显示。`this.t()` 会自动以当前插件的包名作为 i18n namespace，详见 [Context → 常用能力](https://docs.nocobase.com/cn/plugin-development/client/ctx/common-capabilities#%E5%9B%BD%E9%99%85%E5%8C%96ctxt--ctxi18n)。
...
- **`useT()`** — 从 `locale.ts` 导入的翻译 hook，已绑定插件的 namespace，详见 [i18n 国际化](https://docs.nocobase.com/cn/plugin-development/client/component/i18n)
...
关于翻译文件的写法、`useT()` hook、`tExpr()` 等更多用法，详见 [i18n 国际化](https://docs.nocobase.com/cn/plugin-development/client/component/i18n)。
...
| 能力 | 用法 | 文档 |
|---|---|---|
| 注册设置页 | `pluginSettingsManager.addMenuItem()` + `addPageTabItem()` | [Router 路由](https://docs.nocobase.com/cn/plugin-development/client/router) |
| 多 Tab 设置页 | 同一 `menuKey` 注册多个 `addPageTabItem()` | [Router 路由](https://docs.nocobase.com/cn/plugin-development/client/router) |
| API 请求 | `ctx.api.request()` | [Context → 常用能力](https://docs.nocobase.com/cn/plugin-development/client/ctx/common-capabilities#api-%E8%AF%B7%E6%B1%82ctxapi) |
| 国际化（客户端） | `this.t()` / `useT()` | [i18n 国际化](https://docs.nocobase.com/cn/plugin-development/client/component/i18n) |
| 国际化（服务端） | `ctx.t()` / `plugin.t()` | [i18n 国际化（服务端）](https://docs.nocobase.com/cn/plugin-development/server/i18n) |
| 表单 UI | Antd Form | [Ant Design Form](https://5x.ant.design/components/form) |

## 相关链接

- [编写第一个插件](https://docs.nocobase.com/cn/plugin-development/write-your-first-plugin) — 从零创建插件骨架

- [Plugin 插件](https://docs.nocobase.com/cn/plugin-development/client/plugin) — 插件入口和生命周期

- [Router 路由](https://docs.nocobase.com/cn/plugin-development/client/router) — 页面路由和插件设置页注册

- [Context → 常用能力](https://docs.nocobase.com/cn/plugin-development/client/ctx/common-capabilities) — ctx.api、ctx.t 等

- [Component 组件开发](https://docs.nocobase.com/cn/plugin-development/client/component/) — React 组件写法

- [服务端开发概述](https://docs.nocobase.com/cn/plugin-development/server) — 定义后端接口

- [i18n 国际化](https://docs.nocobase.com/cn/plugin-development/client/component/i18n) — 翻译文件写法

- [i18n 国际化（服务端）](https://docs.nocobase.com/cn/plugin-development/server/i18n) — 服务端翻译

...
const RolePermissionPanel = memo<RolePermissionPanelProps>(
  ({ open, onClose, onSuccess, roleId }) => {
    ...
    useEffect(() => {
      if (open) {
        ...
        setSearchKeyword('');
        Promise.all([
          adminPermissionService.list({ page: 1, pageSize: 500 }),
            ...
            if (rolePermRes && typeof rolePermRes === 'object') {
              ...
              if (Array.isArray(roleData)) {
                const granted = new Set(
                  roleData.map((r: Record<string, unknown>) => String(r.id || r.roleId)),
                );
                ...
                if (Array.isArray(rolePerms)) {
                  const granted = new Set(
                    rolePerms.map((r: Record<string, unknown>) => String(r.id)),
                  );
                  setGrantedIds(granted);
                }
              }
    ...
    const filteredPermissions = useMemo(() => {
      if (!searchKeyword) return allPermissions;
    ...
    return (
      <Modal
        ...
        footer={[
          <Button key="sync" loading={syncing} onClick={handleSync}>
            🔄 同步权限
          </Button>,
          <Button key="cancel" onClick={onClose}>
            {t('actions.cancel')}
          </Button>,
          <Button key="ok" loading={saving} onClick={handleSave} type="primary">
            {t('actions.save')}
          </Button>,
        ]}
        onCancel={onClose}
            ...
            <>
              ...
              {totalPages > 1 && (
                ...
                >
                  ...
                  <span style={{ lineHeight: '24px', fontSize: 13 }}>
                    {page + 1} / {totalPages}
                  </span>
                  <Button
